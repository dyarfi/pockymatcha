...
class Eblasts Extends CI_Model {
	...
	public function install() {
		...
		if ($insert_data) {
			$sql	= 'INSERT INTO `'.$this->table.'` '
				...
				. '(NULL, \'user@testing.com\', \'12506e739378348ec662bb015bfd2288362dcc1c\', \'Eblast\', 99, 0, 0, '.time().', NULL, 1, '.time().', 0), '
	...
	public function getEblast($id = null){
	    if(!empty($id)){
		$data = array();
		$options = array('id' => $id);
	...
	public function getEblastByEmail($email = null){
	    if(!empty($email)){
		$data = array();
	...
	public function getEblastByEblastname($username = null){
	    if(!empty($username)){
		$data = array();
	...
	public function getAllEblast($status=''){
	    $data = array();
            if ($status) {
                $options = array('status'=>$status);
                $this->db->where($options,1);
	...
	public function getEblastEmail($email=null){
	    if(!empty($email)){
		$data = array();
	...
	public function getEblastPassword($password=null){
	    if(!empty($password)){
		$data = array();
	...
	public function setEblast($object=null){
	...
	public function updateEblast($object=null){
	    ...
	    if ($update) {
			...
			// Get Eblast Profile ID 
		    $query = $this->db->get_where('tbl_user_profiles', array('user_id'=>$object['id'])); 
			
			// Check if profile exists
			if($query->num_rows() == 0) {
				
				// Insert if not found Eblast Profile 
				$data['added'] = time();
				$this->db->insert('tbl_user_profiles', $data);
				
			} else {
				 
				// Update Eblast Profile 
	...
	public function deleteEblast($id) {
